...
attention_probe="${ATTENTION_PROBE:-0}"
...
for ((i=start_seed; i<start_seed+num_seeds; i++)); do
  dim_zeroout_seed="${i}"

  # This is only the base filename.
  # eval_pretrain.py will append:
  # _zeroout_nonsinkdim_seed_{seed}_numdim_{num}_idx_{...}
  ...
  if [ "${attention_probe}" = "1" ]; then
    extra_args+=(--attention_probe)
  fi

  echo "=================================================="
  echo "Running eval_pretrain.py with dim_zeroout_seed=${dim_zeroout_seed}"
