...
class GraphEncoder(nn.Module):    # first_model, i.e. simple MLP for dimension transformation of OGB/ GIANT node embedding + freezed Llama-7b word embeddings.
    def __init__(self, args, llama_embed):
        ...
        self.graph_projector = nn.Sequential(
            ...
            nn.LayerNorm(self.args.num_token * self.embed_dim)
            )
    ...
    def forward(self, input_ids, is_node, graph):
        ...
        return inputs_embeds # [bsz, seq, dim]  Input to LLM
...
class GraphSAGE(nn.Module):
    def __init__(self, args):
        ...
        if n_layers > 1:
            ...
            self.convs.append(gnn_conv(hidden_channels, out_channels))  # 12.21: Normalization
        else:
            self.convs.append(gnn_conv(in_channels, out_channels))  # 12.21: Normalization
    ...
    def forward(self, x, edge_index, edge_attr=None, batch=None, lp=None):
        ...
        for i in range(len(x)):
            if lp[i].data.item() == True:
                ...
                xs.append(x[i][0])              # x represents the node feature matrix of a batched graphs: x[i][0] denotes the first node: i.e., central node (target node)
        x = t.stack(xs, dim=0)
