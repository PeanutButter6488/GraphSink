...
# run we rename _seed${seed} -> _seed${seed}_run${run} so successive runs
# (and runs at different seeds) each land in their own file.
#
# Final files per run (top2/all/random):
#   ./results/{test_dataset}/{prefix}_prune_{mode}[K]_seed{S}_run{N}_model_results.txt
#   ./results/{test_dataset}/{prefix}_prune_{mode}[K]_seed{S}_run{N}_model_labels.txt
pruning_mode='all'
seed=42
n_runs=1
...
do
    ...
    for run in $(seq 1 "$n_runs"); do
        ...
        # Rename _seed${seed} -> _seed${seed}_run${run} for this run's
        # outputs so successive runs (and runs at different seeds) each
