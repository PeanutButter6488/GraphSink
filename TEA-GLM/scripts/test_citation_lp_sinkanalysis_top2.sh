#!/bin/bash
#
# TEA-GLM LP — sink-token pruning (top-2 mode) on arxiv / cora / pubmed.
# Mirrors scripts/test_citation_sinkanalysis_top2.sh but with --task lp.
#
# Prerequisite: baseline LP run that wrote
#   ./analysis/{test_dataset}_lp/global_stats/{prefix}_sink_records.jsonl
# (i.e. you've already run scripts/test_citation_lp.sh)
#
# Per-run output:
#   ./results/{test_dataset}_lp/{prefix}_prune_top2_seed{S}_run{N}_model_{results,labels}.txt
#   ./analysis/{test_dataset}_lp/global_stats/{prefix}_prune_top2_seed{S}*

export CUDA_VISIBLE_DEVICES=0
export TORCH_DISTRIBUTED_DEBUG=DETAIL
wandb offline

datasets=('arxiv:1024' 'cora:1024' 'pubmed:1024')

dataset='arxiv'           # train-side dataset (controls gnn_input)
num_token=5
prefix='TEA-GLM_citation_meanpool'
llm='./vicuna-7b-v1.5'
best_epoch=49

pruning_mode='top2'
seed=42
n_runs=1

# scripts/test_citation_lp.sh runs the baseline with --append_seed_suffix, so
# the sink records land at {prefix}_seed{baseline_seed}_sink_records.jsonl.
# Override BASELINE_SEED if your baseline used a different seed.
baseline_seed="${BASELINE_SEED:-42}"

case "$pruning_mode" in
    top2)   prune_tag="_prune_top2" ;;
    all)    prune_tag="_prune_all" ;;
    *) echo "Unknown pruning_mode: $pruning_mode" >&2; exit 1 ;;
esac

for pair in "${datasets[@]}"; do
    IFS=':' read -r test_dataset max_text_length <<< "$pair"

    sink_records="./analysis/${test_dataset}_lp/global_stats/${prefix}_seed${baseline_seed}_sink_records.jsonl"
    if [ ! -f "$sink_records" ]; then
        echo "[${test_dataset}] Missing LP sink records: $sink_records"
        echo "Run scripts/test_citation_lp.sh first to produce them. Skipping."
        continue
    fi

    for run in $(seq 1 "$n_runs"); do
        echo "=================================================="
        echo "[LP][${test_dataset}] pruning_mode=${pruning_mode} run=${run} seed=${seed}"
        echo "max_text_length=${max_text_length}"
        echo "=================================================="

        accelerate launch \
            --config_file accelerate_config/config_single_gpu.yaml \
            train_glm.py \
                --freeze_llama \
                --inference \
                --task lp \
                --best_epoch ${best_epoch} \
                --dataset ${dataset} \
                --test_dataset ${test_dataset} \
                --att_d_model 2048 \
                --gnn_output 4096 \
                --grad_steps 1 \
                --batch_size 4 \
                --num_token ${num_token} \
                --clip_grad_norm 1.0 \
                --backbone ${llm} \
                --epoch 1 \
                --weight_decay 0.1 \
                --max_text_length ${max_text_length} \
                --prefix ${prefix} \
                --prune_sink_tokens \
                --pruning_mode ${pruning_mode} \
                --sink_record_path "${sink_records}" \
                --seed ${seed}

        # train_glm.py writes outputs with the natural _prune_{mode}_seed{N}
        # suffix from pruning_output_suffix(); rename _seed${seed} ->
        # _seed${seed}_run${run} so successive same-seed runs land in their
        # own files (matches the NC convention).
        seed_tag="${prune_tag}_seed${seed}"
        run_tag="${prune_tag}_seed${seed}_run${run}"

        for kind in results labels; do
            src="./results/${test_dataset}_lp/${prefix}${seed_tag}_model_${kind}.txt"
            dst="./results/${test_dataset}_lp/${prefix}${run_tag}_model_${kind}.txt"
            if [ -f "$src" ]; then
                mv "$src" "$dst"
                echo "  renamed $(basename "$src") -> $(basename "$dst")"
            else
                echo "  WARNING: expected $src not found; skipping rename for run=${run}"
            fi
        done
    done
done
