...
def get_instructions(data_path):
    ...
    df = df.sample(n=300, ignore_index=True)
...
def preprocess(instruction, tokenizer, max_length, mode='train'):
    ...
    if mode != 'train':
        ...
        targets = tokenizer(
            ...
            return_tensors="pt",
            padding="max_length", # pad input_ids and target_ids
            max_length=100,
            truncation=True,
        ).input_ids
    else:
...
class InstructionDataset(Dataset):
    def __init__(self, tokenizer, args, mode='train') -> None:
        ...
        task_tag = "_LP" if getattr(args, "task", "nc") == "lp" else ""
        if args.inference:
            self.instructions = get_instructions(f"./instruction/{args.test_dataset}/{args.test_dataset}{task_tag}_dataset_{self.mode}.json")
        else:
            self.instructions = get_instructions(f"./instruction/{args.dataset}/{args.dataset}{task_tag}_dataset_{self.mode}.json")
    ...
    def __getitem__(self, idx):    # when doing: InstructionDataset[0]
    ...
    def collate_fn(self, batch):
        ...
        for i, entry in enumerate(batch):
            ...
            node_set_list.append(entry['node_set'])
        ...
        batch_entry['node_set'] = node_set_list
